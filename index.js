...
function addMetadata(packname, author) {	
	if (!packname) packname = 'tukibot'; if (!author) author = 'rosse';	
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hey @${num.split('@')[0]} bienvenido a *${mdata.subject}*, recuerda leer las reglas del grupo para no tener ningun problema.`
                          client.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `Hey @${num.split('@')[0]} se fue👋`
				client.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
/******JUEGOS SHANDUY LA PUTA MADRE NO TE OLVIDES******/					
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'creador':
		...
		client.sendMessage(from, 'Arriba esta mi contacto por si tienen algun problema.',MessageType.text, { quoted: mek} )
		...
		if (!isUser) return reply(mess.only.daftarB)
			...
			case 'tagall':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 MENCIONANDO 〙✪══\n╠➥'+teks+'╚═〘 by tukibot 〙', members_id, true)
					break
                                case 'send':
					var pc = body.slice(6)
...
client.groupRemove(from, mentioned)
client.sendMessage(mentioned, 'Chaoa', text)
const none = fs.readFileSync('./mp3/baneado.mp3');
client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
}
break
...
//FIN DE REGISTRO  
		...
		if (budy.includes(`patas`)) {
                  reply(`las patas son ricas \nnashe`)
                  }
